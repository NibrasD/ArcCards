...
async def pay_order_onchain(order_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not pk or not rpc:
            raise Exception("Missing ARC_RPC_URL or AGENT_PRIVATE_KEY. Real transactions require these environment variables.")
        ...
        try:
            ...
            print(f"Approve failed/skipped: {e}")
            nonce = w3.eth.get_transaction_count(account.address)

        # 2. Call pay_for_order
        ...
        return {"status": "success", "tx_hash": real_tx_hash, "message": "Real Arc transaction sent successfully!"}

    except Exception as overall_e:
        print(f"[REAL MODE ERROR] {overall_e}")
        raise HTTPException(status_code=500, detail=f"On-Chain Payment Failed: {str(overall_e)}")
